...
import { Award, Flame, TrendingUp, Trophy, ChevronRight, Calendar, MessageSquare } from "lucide-react"
...
const badges = [
  ...
  },
  {
    ...
    longDescription: "Consistency is key! Post memes for 7 days in a row to earn this badge. Show your dedication!",
    requirement: 7,
...
export function RightSidebar() {
  ...
  useEffect(() => {
    const fetchTopContent = async () => {
      ...
      try {
        // Fetch top memes
        const memesResponse = await axios.get(`/api/posts/trending?timeFrame=${timeFrame}`)
        ...
        // Fetch top users
        try {
          const usersResponse = await axios.get(`/api/users/top?timeFrame=${timeFrame}`)
          if (usersResponse.data.success) {
            // Transform the data to match the TopUser interface
            const formattedUsers = usersResponse.data.data.map((userData: any) => ({
              id: userData._id,
            ...
            setTopUsers([
              {
                ...
                likes: userProfile?.posts?.reduce((sum: number, post: any) => sum + post.likes, 0) || 0,
              },
            ])
          }
        }
  ...
  const isStreakActive = lastPostDate ? new Date(lastPostDate).toDateString() === new Date().toDateString() : false
      ...
      <Card className="border shadow-sm bg-card/50 backdrop-blur-sm">
        ...
        <CardContent className="p-4 pt-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="rounded-md bg-muted/50 p-2">
              <div className="text-sm font-medium">{userProfile?.posts?.length || 0}</div>
              <div className="text-xs text-muted-foreground">Posts</div>
            </div>
            <div className="rounded-md bg-muted/50 p-2">
              <div className="text-sm font-medium">{userProfile?.user?.followers || 0}</div>
          ...
          <div className="mt-4 flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-orange-500/10 to-red-500/10 border border-orange-500/20">
            ...
            <div className="text-sm">
              <div className="font-bold text-orange-500">{streak}-day streak!</div>
              <div className="text-xs text-muted-foreground">
                {isStreakActive ? "You've posted today!" : "Post today to keep your streak going!"}
              </div>
            </div>
          </div>
        ...
        <div className="px-4 pb-4">
          ...
          {earnedBadges.length > 0 && (
            <div className="mb-4">
              <div className="text-xs text-muted-foreground mb-2">Earned Badges</div>
                ...
                <TooltipProvider>
                  {earnedBadges.map((badge) => (
                    <Tooltip key={badge.id}>
                      ...
                      <TooltipContent side="bottom" className="max-w-[200px] animate-fade-in">
          ...
          <div className="space-y-3">
            <div className="text-xs text-muted-foreground">Available Badges</div>
            {badges
              ...
              .map((badge) => {
                ...
                if (badge.id === "meme-lord") {
                  ...
                  progress = Math.min((totalLikes / badge.requirement) * 100, 100)
                  ...
                  progress = Math.min((totalComments / badge.requirement) * 100, 100)
                ...
                return (
                  <div key={badge.id} className={`p-3 rounded-lg ${badge.bgColor} border ${badge.borderColor}`}>
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        <div className={`text-sm font-bold ${badge.textColor}`}>{badge.name}</div>
                    ...
                    {badge.id === "award-winner" && (
                      <div className="text-xs text-muted-foreground mt-1">Get featured in top memes of the week</div>
                    )}
                  </div>
                )
              })}
          ...
          <Tabs defaultValue="memes" className="w-full">
              ...
              <div className="px-4 pb-4">
                {topMemes.length === 0 ? (
                  <div className="flex flex-col items-center justify-center p-6 text-center">
                    <p className="text-muted-foreground">No memes to show yet</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {topMemes.slice(0, 5).map((meme, index) => (
                      ...
                      >
                        <div className="relative">
                          <img
                            src={meme.image || "/placeholder.svg?height=100&width=300"}
                            alt={meme.title}
                          ...
                          <div className="absolute bottom-0 left-0 right-0 p-2 flex items-center gap-2">
                            ...
                            <div className="flex-1 min-w-0 text-left">
                              <div className="text-xs font-medium text-white truncate">{meme.title}</div>
                              <div className="text-[10px] text-white/80">@{meme.username}</div>
                            </div>
            ...
            <TabsContent value="users" className="mt-0">
              <div className="px-4 pb-4">
                {topUsers.length === 0 ? (
                  <div className="flex flex-col items-center justify-center p-6 text-center">
                    <p className="text-muted-foreground">No users to show yet</p>
                  </div>
                ) : (
