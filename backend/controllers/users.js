...
exports.followUser = async (req, res) => {
  try {
    ...
    const isFollowing = req.user.following.some((id) => id.toString() === userToFollow._id.toString())
    ...
    try {
      ...
      if (isFollowing) {
        await User.findByIdAndUpdate(userToFollow._id, { $pull: { followers: req.user.id } }, { session })

        await User.findByIdAndUpdate(req.user.id, { $pull: { following: userToFollow._id } }, { session })
      ...
      await User.findByIdAndUpdate(userToFollow._id, { $addToSet: { followers: req.user.id } }, { session })

      await User.findByIdAndUpdate(req.user.id, { $addToSet: { following: userToFollow._id } }, { session })
...
exports.getUserFollowers = async (req, res) => {
  try {
    ...
    const followersWithStatus = user.followers.map((follower) => {
      const isFollowing = req.user.following.some((id) => id.toString() === follower._id.toString())
      return {
        id: follower._id,
...
exports.getUserFollowing = async (req, res) => {
  try {
    ...
    const followingWithStatus = user.following.map((followedUser) => {
      const isFollowing = req.user.following.some((id) => id.toString() === followedUser._id.toString())
      return {
        id: followedUser._id,
...
exports.updateUserStreak = async (userId) => {
  try {
    ...
    if (!lastStreakUpdate || lastStreakUpdate.getTime() !== today.getTime()) {
      ...
      } else if (!lastStreakUpdate || lastStreakUpdate.getTime() < yesterday.getTime()) {
...
exports.debugRelationships = async (req, res) => {
  try {
    const user = await User.findById(req.user.id).populate("followers", "username").populate("following", "username")
